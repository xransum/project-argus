"""DNS Lookup bulk API endpoints — accepts POST with a list of domains or IPs."""
...
async def bulk_dns_lookup(body: TargetBulkRequest) -> JobCreatedResponse:
    """Perform DNS lookups for a list of domains or IPs and return a job ID."""
    sanitised = _validate_targets(body.targets)
    job_id = await enqueue_job("dns/lookup", sanitised)
