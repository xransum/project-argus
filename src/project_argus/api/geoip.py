"""GeoIP Lookup bulk API endpoints — accepts POST with a list of domains or IPs."""
...
async def bulk_geoip_lookup(body: TargetBulkRequest) -> JobCreatedResponse:
    """Resolve GeoIP data for a list of domains or IPs and return a job ID."""
    sanitised = _validate_targets(body.targets)
    job_id = await enqueue_job("geoip/lookup", sanitised)
    return JobCreatedResponse(
        job_id=job_id,
        job_type="geoip/lookup",
        status="pending",
        total=len(sanitised),
