"""Blacklist Check bulk API endpoints — accepts POST with a list of domains or IPs."""
...
async def bulk_blacklist_check(body: TargetBulkRequest) -> JobCreatedResponse:
    """Check blacklist status for a list of domains or IPs and return a job ID."""
    sanitised = _validate_targets(body.targets)
    job_id = await enqueue_job("blacklist/check", sanitised)
    return JobCreatedResponse(
        job_id=job_id,
        job_type="blacklist/check",
        status="pending",
        total=len(sanitised),
