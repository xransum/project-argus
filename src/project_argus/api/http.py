"""HTTP/URL bulk API endpoints — accepts POST with a list of URLs."""
...
async def bulk_http_status(body: URLBulkRequest) -> JobCreatedResponse:
    ...
    job_id = await enqueue_job("http/status", sanitised)
...
async def bulk_http_headers(body: URLBulkRequest) -> JobCreatedResponse:
    ...
    sanitised = _validate_urls(body.urls)
    job_id = await enqueue_job("http/headers", sanitised)
    return JobCreatedResponse(
        job_id=job_id,
        job_type="http/headers",
        status="pending",
        total=len(sanitised),
        message="Job enqueued. Poll /api/jobs/{job_id} for progress.",
    )
