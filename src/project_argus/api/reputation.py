"""Reputation Check bulk API endpoints — accepts POST with a list of domains or IPs."""
...
async def bulk_reputation_check(body: TargetBulkRequest) -> JobCreatedResponse:
    """Check reputation for a list of domains or IPs and return a job ID."""
    sanitised = _validate_targets(body.targets)
    job_id = await enqueue_job("reputation/check", sanitised)
    return JobCreatedResponse(
        job_id=job_id,
        job_type="reputation/check",
        status="pending",
        total=len(sanitised),
